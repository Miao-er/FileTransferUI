...
int HDRDMA_SUPPORT_THREADS = 2;
int HDRDMA_LOCAL_PORT = 10471;
int HDRDMA_REMOTE_PORT = 10470;
int HDRDMA_GID_INDEX = 1;
double HDRDMA_SEND_RATE = 100;
std::string HDRDMA_REMOTE_ADDR = "";
std::string HDRDMA_MIDDLE_ADDR = "192.168.2.2";
...
void recvData(HwRdma *hwrdma, int peer_fd)
{
    LucpOnRdma lucp_controller(hwrdma, peer_fd, HDRDMA_SEND_RATE);
    std::shared_ptr<int> x(NULL, [&](int *)
                           { close(peer_fd); });
    IPInfo ip_info = {"192.168.7.10",
                      "192.168.6.20",
                      "192.168.2.2"};
    if (lucp_controller.bindMemoryRegion(HDRDMA_BUFF_LEN_KB / HDRDMA_SUPPORT_THREADS * 1024) == -1)
        return;
    if (lucp_controller.createBufferPool(HDRDMA_NUM_BUFF_SECTIONS) == -1)
        return;
    if (lucp_controller.createLucpContext(ip_info) == -1)
        return;
    if (lucp_controller.connectPeer() == -1)
        return;
    if (lucp_controller.postRecvFile() == -1)
        return;
}
void sendData(HwRdma *hwrdma, int peer_fd)
{
    ...
    if (lucp_controller.connectPeer() == -1)
        return;
    if (lucp_controller.postSendFile(HDRDMA_SRCFILENAME.c_str(), HDRDMA_DSTFILENAME.c_str()) == -1)
        return;
    HDRDMA_DSTFILENAME = HDRDMA_DSTFILENAME + "2";
}
//-------------------------------------------------------------
// main
//-------------------------------------------------------------
int main(int narg, char *argv[])
{
    ...
    HwRdma hwrdma(HDRDMA_GID_INDEX, 1, HDRDMA_BUFF_LEN_KB * 1024);
    hwrdma.init();
    ...
    if (HDRDMA_IS_SERVER)
    {
        ...
        addr.sin_port = htons(HDRDMA_LOCAL_PORT);
        ...
        cout << "Listening for connections on port ... " << HDRDMA_LOCAL_PORT << endl;
        ...
        while (1)
        {
            ...
            if (peer_sockfd > 0)
            {
                ...
                std::thread thr(recvData, &hwrdma, peer_sockfd);
                thr.detach();
            }
            else
            {
...
void ParseCommandLineArguments(int narg, char *argv[])
{
    ...
    for (int i = 1; i < narg; i++)
    {
        ...
        {
            HDRDMA_SUPPORT_THREADS = atoi(next.c_str());
            i++;
        }
        else if (arg == "-p")
        {
        ...
        {
            HDRDMA_GID_INDEX = atoi(next.c_str());
            i++;
        }
        else if (arg == "-r")
        {
            HDRDMA_SEND_RATE = atof(next.c_str());
            i++;
        }
        else if (arg[0] != '-')
        {
    ...
    if (!HDRDMA_IS_SERVER)
    {
        ...
        if (HDRDMA_BUFF_LEN_KB == 0)
            HDRDMA_BUFF_LEN_KB = 32;
        if (HDRDMA_NUM_BUFF_SECTIONS == 0)
            HDRDMA_NUM_BUFF_SECTIONS = 4;
    }
    else
    {
        if (HDRDMA_BUFF_LEN_KB == 0)
            HDRDMA_BUFF_LEN_KB = 64;
        if (HDRDMA_NUM_BUFF_SECTIONS == 0)
            HDRDMA_NUM_BUFF_SECTIONS = 8;
    }
}
...
void Usage(void)
{
    ...
    cout << "	 -g         set used gid index of device(default 1)." << endl;
    cout << endl;
    cout << "NOTES:" << endl;
